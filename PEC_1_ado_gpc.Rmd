...
```
```{r}
topTabTiempo  <- topTable (fit.main, number=nrow(fit.main) , coef="Diet", adjust="fdr")
```
...
Una forma más visual de analizar estos resultados es mediante un diagrama de Venn:

```{r}
vennDiagram (res.selected, main="Genes in common #1", cex=0.9)
```
